          ...
          <div className="bg-white rounded-3xl border border-stone-200 shadow-sm p-6 space-y-4">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold text-stone-900">Integração Perenual API (Classificação Botânica)</h3>
              </div>
              {perenualService.isKeyFromEnv() ? (
                <span className="text-[11px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-300 px-2.5 py-1 rounded-full flex items-center gap-1">
                  🔒 Chave segura via .env / Host Secrets
                </span>
              ) : (
                <span className="text-[11px] font-semibold bg-stone-100 text-stone-600 border border-stone-200 px-2.5 py-1 rounded-full">
                  Armazenamento Local
                </span>
              )}
            </div>
            <p className="text-xs text-stone-500">
              A chave da API é usada para pesquisar tanto por <strong>Nome Popular</strong> quanto por <strong>Nome Científico</strong> e classificar automaticamente suas plantas.
            </p>
            <div className="space-y-3">
              {perenualService.isKeyFromEnv() ? (
                <div className="bg-emerald-50/70 border border-emerald-200 rounded-2xl p-4 text-xs text-emerald-900 space-y-1">
                  <p className="font-bold">✅ Chave de API Configurada com Sucesso</p>
                  <p className="text-emerald-800">
                    Sua chave está carregada com segurança através da variável de ambiente <code className="font-mono bg-emerald-100 px-1 py-0.5 rounded text-[11px]">VITE_PERENUAL_API_KEY</code> fora do repositório.
                  </p>
                </div>
              ) : (
                <div>
                  <label className="block text-xs font-semibold text-stone-700 mb-1">
                    Chave de API Perenual (ou configure <code className="font-mono text-stone-600">VITE_PERENUAL_API_KEY</code> no seu arquivo .env)
                  </label>
              ...
              <p className="text-[11px] text-stone-500">
                Para manter a chave 100% fora do código em produção, crie o segredo <code className="font-mono bg-stone-100 px-1 py-0.5 rounded text-[11px]">VITE_PERENUAL_API_KEY</code> nas configurações de ambiente do seu host (Vercel, Netlify, Render) ou no arquivo <code className="font-mono bg-stone-100 px-1 py-0.5 rounded text-[11px]">.env</code> local.
              </p>
            </div>
          </div>
