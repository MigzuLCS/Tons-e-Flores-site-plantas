...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  // Estado de configurações (categorias, bancadas e cultivos)
  ...
  const [cultivations, setCultivations] = useState<any[]>(() => configService.getCultivationDetails());
  ...
  useEffect(() => {
    ...
    configService.syncCultivationsWithCloud().then(setCultivations);
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-12 animate-in fade-in">
      ...
      {adminTab === 'config' && (
        <div className="space-y-6 animate-in fade-in">
          ...
          {/* Tipos de Cultivo (Supabase Sync) */}
          <div className="bg-brand-surface rounded-3xl border border-brand-border shadow-xs p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Sprout className="w-5 h-5 text-brand-olive" />
                <h3 className="text-base font-bold text-brand-text flex items-center gap-2">
                  Tipos de Cultivo & Apresentação
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-brand-olive-light text-brand-olive-text font-bold border border-brand-olive-border">
                    ☁️ Nuvem Supabase
                  </span>
                </h3>
              ...
              <span className="text-xs text-brand-text-muted">
                <strong>{cultivations.length}</strong> tipos de cultivo ativos
              </span>
            </div>
            <p className="text-xs text-brand-text-muted">
              Define a estrutura ou modalidade de cultivo da planta (ex: Tradicional em vaso, Muda, Bonsai, Arranjo, Kokedama). O valor padrão para novas plantas é <strong>Tradicional</strong>.
            </p>

            {/* Lista de cultivos */}
            <div className="flex flex-wrap gap-2">
              {cultivations.map(cul => (
                <div key={cul.id || cul.name} className="flex items-center gap-1.5 bg-brand-surface-subtle border border-brand-border rounded-xl px-3 py-1.5 text-xs font-semibold text-brand-text">
                  <span>{cul.name}</span>
                  {cul.name !== 'Tradicional' && (
                    <button
                      onClick={async () => {
                        const updated = await configService.removeCultivation(cul.name);
                        setCultivations(updated);
                      }}
                      title="Remover tipo de cultivo"
                      className="text-brand-text-muted hover:text-brand-nude-text cursor-pointer transition-colors"
                    >
                      <XIcon className="w-3 h-3" />
                    </button>
                  )}
            ...
            {/* Adicionar novo tipo de cultivo */}
            <div className="flex gap-2 items-center">
              <input
                type="text"
                value={newCultName}
                onChange={e => setNewCultName(e.target.value)}
                placeholder="Ex: Terrário, Jardim Vertical, Placa..."
                className="flex-1 px-3 py-2 text-xs bg-brand-surface-subtle border border-brand-border rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-olive text-brand-text"
                onKeyDown={async e => {
                  if (e.key === 'Enter' && newCultName.trim()) {
                    const updated = await configService.addCultivation(newCultName.trim());
                    setCultivations(updated);
                    setNewCultName('');
                  }
                }}
              ...
              <button
                onClick={() => setCultivations(configService.resetCultivations())}
                title="Restaurar tipos de cultivo padrão"
                className="px-3 py-2 bg-brand-surface-subtle hover:bg-brand-border text-brand-text text-xs font-semibold rounded-xl cursor-pointer border border-brand-border flex items-center gap-1"
              >
                <RotateCcw className="w-3.5 h-3.5" />
                Restaurar padrão
